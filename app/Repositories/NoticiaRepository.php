...
class NoticiaRepository
{
    ...
    {
        $sql = "
        SELECT DISTINCT TRIM(SUBSTRING_INDEX(SUBSTRING_INDEX(categorias, ',', n.n), ',', -1)) AS claves
        FROM not_noticia
        CROSS JOIN (SELECT @rownum := @rownum + 1 AS n FROM information_schema.columns, not_noticia LIMIT 1000) n
        WHERE publicar = 1
        AND estado = 'AC'
        ORDER BY claves ASC;
    ";
    ...
    {
        $sql = "
        SELECT DISTINCT claves
        FROM (
            SELECT 
                TRIM(SUBSTRING_INDEX(SUBSTRING_INDEX(lower(palabras_clave), ',', n.n), ',', -1)) AS claves
            FROM 
                not_noticia
            JOIN 
            ...
            ON CHAR_LENGTH(palabras_clave) - CHAR_LENGTH(REPLACE(palabras_clave, ',', '')) >= n.n - 1
            WHERE publicar = 1
            AND estado = 'AC'
        ) AS m1
        ORDER BY claves ASC;
