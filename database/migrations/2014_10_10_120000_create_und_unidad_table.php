...
class CreateUndUnidadTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('und_unidad', function (Blueprint $table) {
            ...
            $table->unsignedBigInteger('bio_id'); // Asegúrate que esto sea unsignedBigInteger
            $table->unsignedBigInteger('und_padre_id')->nullable(); // Asegúrate que esto sea unsignedBigInteger
            $table->unsignedBigInteger('tiu_id'); // Asegúrate que esto sea unsignedBigInteger

            // Define las claves foráneas correctamente sin dropForeign
